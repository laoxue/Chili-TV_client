<template class="login_no">
  ...
  <div id="login" class="width-vertical">
    <Loading :options="loading_type" v-if="model"/>
    <!-- 内层 -->
    <div class="login-content nc-outer-box">
      <div class="login-password">
        ...
        <div id="login-form" class="login-form">
          ...
          <!-- 登录方式 -->
          <div
            class="login-blocks login-links"
            style="
              display: flex;
              padding: 0 30px;
            ...
            "
          >
            <span target="_self" class="sms-login-link">短信验证码登录</span
            ><span class="register-link" @click="design">免费注册</span>
          </div>
          <div class="fm-btn">
            <button
              type="button"
              ...
              style="width: 85%; margin-left: -0.5%"
              @click="login"
              id="TencentCaptcha"
...
<script>
import Loading from "@/components/common/loading";  // loading组件
...
#login .fm-button {
  background: -webkit-linear-gradient(left, rgb(255 144 0), rgb(255 80 0))
    no-repeat;
